...
/**
 * An instance of this is created whenever `Call#prompt` is called successfully and then added to `Call#prompts` and removed once the prompt is
 ...
 */
class Prompt {
  constructor(user, channel, options, resolve, reject, Call) {
  ...
  /**
    ...
    const index = this.Call.prompts.findIndex((p) => p.user.id === this.user.id && p.channel.id === this.channel.id);

    if (index !== -1)
      this.Call.prompts.splice(index, 1);
